...
def youhuaTspfunction(points):

    current_position = np.array(points[0])
    ...
    while unvisited:
        ...
        for idx in unvisited:
            dist = np.linalg.norm(current_position[:2] - points[idx][:2])
            if dist < min_dist:
                min_dist = dist
                nearest_idx = idx

        path_indices.append(nearest_idx)
        current_position = points[nearest_idx]
...
def arg_parse():
    ...
    parser.add_argument('--iter_max', type=int, default= 2000 ) 
    parser.add_argument('--clearance', type=float, default=3, help='0 for block and gap, 3 for random_2d.')
    parser.add_argument('--pc_n_points', type=int, default=100)
    parser.add_argument('--pc_over_sample_scale', type=int, default=5)
    parser.add_argument('--pc_sample_rate', type=float, default=0.5)
    parser.add_argument('--pc_update_cost_ratio', type=float, default=0.9)
    parser.add_argument('--connect_max_trial_attempts', type=int, default=5)
        ...
        while (True):
            try:
                ...
                if (flag == 1):
                    ...
                    tsp_usestoreold2 = np.array(tsp_usestoreold1[::-1])

                    print(tsp_usestoreold2)

                    tsp_usestoreold = youhuaTspfunction(tsp_usestoreold2)
                    ...
                    for i in range(0, s1 - 1):
                        ...
                        interpolated_path = np.vstack(
                            [interpolated_path, tsp_usestoreold[i + 1]])
...
if __name__ == '__main__':
    global flag,t1,pi,prvrad, prvnz,cirobstacle1,recobstacle1,scale_factor,robotCon,static_person
    flag = 1
    t1 = time.time()
    args = arg_parse()
    # * sanity check
