...
from path_planning_classes.dsgrrt_star_png_2d import DSGRRTStarPNG2D
from path_planning_classes.rrt_visualizer_2d import DSGRRTStarVisualizer
from datasets.point_cloud_mask_utils import generate_rectangle_point_cloud, \
    ellipsoid_point_cloud_sampling


class DSGRRTStarPNGC2D(DSGRRTStarPNG2D):
    def __init__(
        self,
        x_start,
        x_goal,
        ...
        self.visualizer = DSGRRTStarVisualizer(self.x_start, self.x_goal, self.env)
    ...
    def visualize(self, x_center, c_best, start_goal_straightline_dist, theta, figure_title=None,img_filename=None):
        if figure_title is None:
            #figure_title = "dsgrrt*(c) 2D, iteration " + str(self.iter_max)
            figure_title = "dsgrrt(c) 2D, iteration " + str(self.iter_max)
        if img_filename is None:
            #img_filename="dsgrrt*_c_2d_example.png"
            img_filename1 = "dsgrrt_c_2d_example.png"
            # 生成一个 1 到 10000 之间的随机数字
            random_number = random.randint(1, 10000)

            # 获取文件名和扩展名
            base_name, extension = img_filename1.rsplit('.', 1)
            # 拼接新的文件名
            img_filename = f"{base_name}_{random_number}.{extension}"
