...
#scenario 3 setting
robotCon = np.array([3.61,7.61,pi/2])


static_person1 = pd.read_csv(r"D:\DSGRRT\simulation\dsgrrt\scenario_3\dsgrrt_static_person.csv", header=0)
static_person = np.array(static_person1.values)

print(static_person)

cirobstacle = pd.read_csv(r"D:\DSGRRT\simulation\dsgrrt\scenario_3\dsgrrt_cirobstacle.csv", header=0)
cirobstacle1 = np.array(cirobstacle.values)

recobstacle = pd.read_csv(r"D:\DSGRRT\simulation\dsgrrt\scenario_3\dsgrrt_recobstacle.csv", header=0)
...
robotCon = np.array(combined_data[-1])

print(robotCon)

static_person1 = pd.read_csv(r"D:\DSGRRT\simulation\dsgrrt\scenario_3\dsgrrt_static_person1.csv", header=0)
static_person = np.array(static_person1.values)
static_person2 = pd.read_csv(r"D:\DSGRRT\simulation\dsgrrt\scenario_3\dsgrrt_static_person2.csv", header=0)
static_person21 = np.array(static_person2.values)

t1 = time.time()
tsp_usestoreold1 , regpold1,regnzold1 = GroupsPerceptionPart3(robotCon,static_person,prvrad,prvnz)
tsp_usestoreold = np.array(tsp_usestoreold1[::-1])

tsp_usestoreold2 , regpold2,regnzold2 = GroupsPerceptionPart3(robotCon,static_person21,prvrad,prvnz)


regp = [[x * scale_factor, y * scale_factor, r * scale_factor] for x, y, r in regpold2]
regnz = [[x * scale_factor, y * scale_factor, r * scale_factor] for x, y, r in regnzold2]
...
for i in range(0,3):
    ...
    #path = path_planner.planning(visualize=True)/25
    path1 = path_planner.planning(visualize=True) / 20
    interpolated_path1 = interpolate_directions(path1)

    interpolated_path1 = np.vstack([interpolated_path1, tsp_usestoreold[i + 1]])
    newpath1.append(interpolated_path1)

combined_data1 = np.vstack(newpath1)

print(combined_data1)

combined_data = pd.DataFrame(combined_data, columns=['x', 'y', 'theta'])
combined_data1 = pd.DataFrame(combined_data1, columns=['x', 'y', 'theta'])

combined_data2 = pd.concat([combined_data, combined_data1], axis=0, ignore_index=True)

print(time.time() - t)

data1 = pd.DataFrame(combined_data, columns=['x', 'y', 'theta'])

data1.to_csv("D:/DSGRRT/simulation/dsgrrt/scenario_3/dsgrrt_newpath_1a.csv", index=False)

data1 = pd.DataFrame(combined_data2, columns=['x', 'y', 'theta'])

data1.to_csv("D:/DSGRRT/simulation/dsgrrt/scenario_3/dsgrrt_newpath_1.csv", index=False)
